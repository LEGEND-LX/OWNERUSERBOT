...
DELETE_TIMEOUT = 5
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "『DragonRoẞø†』"
legend = bot.uid
...
async def install(event):
    ...
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            ...
            await edit_or_reply(
                LEGEND_file,
                f"**Output Too Large. This is the file for the list of plugins in DragonRoẞø✞︎**.\n\nBY :- **{DEFAULTUSER}**",
            )
            await event.delete()
...
CmdHelp("core").add_command(
    ...
    "cmds", None, "Gives out the list of modules in DragonRoBot."
).add_warning(
