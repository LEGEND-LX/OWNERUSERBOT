...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "DRAGONROBOOT_help":
            rev_text = query[::-1]
            ...
            fsub_btn = [
                [
                    Button.url(
                        f"{LEGEND_USER}", f"tg://openmessage?user_id={Drak_Legend_Pro}"
                    )
                ],
                [
                    Button.url("📍My Channel📍", f"https://t.me/{my_channel}"),
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚜ Legendary Af Pro - DragonRoBoot ⚜**",
                buttons=[
                    [Button.url("♥️ Tutorial ♥", "https://youtu.be/9dQgdUJfk_k")],
                    [Button.url("📍 𝚁𝚎𝚙𝚘 📍", "https://github.com/DARK-LEGEND-PRO/PRO-DRAGONSETUP")],
                    [
                        Button.url(
                            "💞 Deploy 💞",
            ...
            buttons = [
                (
                    Button.url("Sources", "https://github.com/DARK-LEGEND-PRO/PRO-DRAGONSETUP"),
                    Button.url(
                        "Deploy",
                        "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FPROBOY-OP%2FPRO-LEGENDBOT&template=https%3A%2F%2Fgithub.com%2FPROBOY-OP%2FPRO-LEGENDBOT",
                    ),
            ...
            text, msg_entities = await event.client._parse_message_text(
                "𝗗𝗘𝗣𝗟𝗢𝗬 𝗬𝗢𝗨𝗥 𝗢𝗪𝗡 𝗣𝗥𝗢 DRAGONROBOOT", "md"
            )
            result = types.InputBotInlineResult(
                id=str(uuid4()),
                type="photo",
                title="PRO-DRAGONROBOOT",
                description="Deploy yourself",
                url="https://github.com/DARK-LEGEND-PRO/PRO-DRAGONSETUP",
                thumb=photo,
                content=photo,
    ...
    async def on_pm_click(event):
        if event.query.user_id == bot.uid:
            ...
            await event.edit(
                f"🔰 This is Pro-DrgonRoẞø† PM Security for {legend_mention} to keep away unwanted retards from spamming PM..."
            )
    ...
    async def reopn(event):
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            ...
            reply_pop_up_alert = "This Is For My Master Only.Dont Try To Touch Again. Deploy Ur Own ©Pro-DragonRoẞø†™"
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            ...
            await event.edit(
                f"My Master {Config.ALIVE_NAME} has Been Closed Menu\n\n               [©️DragonRoẞø†]({chnl_link})",
                buttons=veriler,
                link_preview=False,
            )
        else:
            await event.answer(
                "Deploy Ur Own     ©Pro-DragonRoẞø†", cache_time=0, alert=True
            )
