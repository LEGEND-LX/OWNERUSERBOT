...
msg = f"""
**⚜ 𝙻𝚎𝚐𝚎𝚗𝚍𝚊𝚛𝚢 𝙰𝚏 DragoNRo𝙱𝚘o𝚝 ⚜**

  •        [♥️ 𝚁𝚎𝚙𝚘 ♥️](https://github.com/DRAK-LEGEND-PRO/PRO-DRAGONSETUP)
...
@bot.on(admin_cmd(pattern="deploy$"))
@bot.on(sudo_cmd(pattern="deploy$", allow_sudo=True))
async def repo(event):
    try:
        legend = await bot.inline_query(botname, "deploy")
...
@bot.on(admin_cmd(pattern="roboot ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="roboot ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if tgbotusername is not None:
        try:
            results = await event.client.inline_query(tgbotusername, "DRAGONROBOOT_help")
            ...
            async with bot.conversation(chat) as conv:
                ...
                await legend.edit(
                    f"**Turned On Inline Mode Successfully.** \n\nDo `{l1}roboot` again to get the help menu."
                )
...
async def _(event):
    ...
    await event.edit("тєℓєтнση  вαѕє∂ υѕєявσт ρσωєяє∂ ву **DragonRoẞOO†** вσт")
